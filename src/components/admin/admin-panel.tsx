...
const emptyProduct: ProductInput = {
  ...
  instruccionesCuidado:
    "Lavado a mano con agua tibia. No usar blanqueador. Secar a la sombra.",
  diasEntrega: 7,
  artesanal: true,
  stock: 10,
  peso: 150,
...
function StatusBadge({ status }: { status: AdminOrderStatus }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${colors[status]}`}
    >
...
function Modal({
  ...
  return (
    <div
      className="fixed inset-0 z-100 flex items-end justify-center bg-black/60 p-0 backdrop-blur-xs sm:items-center sm:p-5"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget) onClose();
      }}
    >
      <section
        className={`max-h-[92vh] w-full overflow-y-auto rounded-t-4xl bg-white p-5 shadow-2xl sm:rounded-4xl sm:p-7 ${wide ? "max-w-4xl" : "max-w-xl"}`}
      >
        <div className="mb-6 flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-gray-100"
          >
            <FiX />
          </button>
        </div>
...
export default function AdminPanel() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error(cause);
      setError(
        "No se pudo cargar el panel. Verifica que ejecutaste neon-admin-panel.sql en la rama correcta de Neon.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleArchive = async (product: Product) => {
    ...
    try {
      ...
      setProducts((current) =>
        current.map((item) =>
          item.id === product.id ? { ...item, isActive: active } : item,
        ),
      );
      notify(
        active
          ? "Producto restaurado."
          : "Producto retirado del catálogo sin borrar su historial.",
      );
    } catch {
  ...
  const handleFeatured = async (product: Product) => {
    ...
    try {
      ...
      setError("No se pudo actualizar Productos del mes.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleBan = async (account: AdminUser) => {
    ...
    try {
      await setUserBan(
        account.userId,
        !account.banned,
        account.banned ? undefined : "Bloqueado desde el panel administrativo",
      );
      setUsers((current) =>
        current.map((item) =>
    ...
    return (
      <div className="grid gap-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          <label className="relative flex-1">
            ...
            <input
              ...
              className="h-11 w-full rounded-full border bg-white pl-11 pr-4 text-sm outline-none focus:border-violet-400"
            />
          </label>
          <select
            value={statusFilter}
        ...
        <div className="overflow-hidden rounded-3xl border bg-white">
          <div className="hidden grid-cols-[90px_1.3fr_1fr_130px_120px_44px] gap-4 border-b bg-gray-50 px-5 py-3 text-xs font-bold uppercase tracking-wide text-gray-500 lg:grid">
            <span>Pedido</span>
            <span>Cliente</span>
            <span>Fecha</span>
            <span>Estado</span>
          ...
          {source.length === 0 ? (
            ...
            source.map((order) => (
              <button
                ...
                className="grid w-full cursor-pointer gap-3 border-b px-5 py-4 text-left last:border-0 hover:bg-violet-50/40 lg:grid-cols-[90px_1.3fr_1fr_130px_120px_44px] lg:items-center lg:gap-4"
              >
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fb] text-gray-950">
      <aside
        className={`fixed inset-y-0 left-0 z-80 flex w-72 flex-col border-r border-violet-100 bg-white p-5 transition-transform lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between">
          <span className="font-monoton text-xl">Bearys</span>
          <button
        ...
        <div className="mt-8 rounded-3xl bg-violet-50 p-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex cursor-pointer items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold ${section === item.id ? "bg-violet-600 text-white" : "text-gray-600 hover:bg-violet-50"}`}
              >
                <Icon />
                {item.label}
              </button>
        ...
        <div className="mt-auto grid gap-2">
          <Link
            to="/"
            className="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-gray-600 hover:bg-gray-50"
          >
            <FiHome /> Ver tienda
          </Link>
          <button
            onClick={() => void logout()}
            className="flex cursor-pointer items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-red-600 hover:bg-red-50"
          >
      ...
      <main className="min-h-screen lg:pl-72">
        <header className="sticky top-0 z-50 flex h-18 items-center justify-between border-b border-violet-100 bg-white/90 px-4 backdrop-blur-md sm:px-7 lg:px-10">
          ...
          <button
            ...
            disabled={loading}
            className="flex cursor-pointer items-center gap-2 rounded-full border bg-white px-4 py-2 text-sm font-semibold"
          >
        ...
        <div className="mx-auto max-w-400 p-4 sm:p-7 lg:p-10">
          ...
          {message && (
            <div className="fixed bottom-5 right-5 z-110 flex items-center gap-2 rounded-full bg-gray-950 px-5 py-3 text-sm font-semibold text-white shadow-xl">
              <FiCheck className="text-emerald-400" />
              {message}
            </div>
          )}
          {loading ? (
            <div className="flex min-h-100 items-center justify-center text-violet-600">
            ...
            <>
              ...
              {section === "pedidos" && (
                <section>
                  <div className="mb-6">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Actualiza el avance, revisa la entrega o cancela un
                      pedido.
                    </p>
                  </div>
                  {renderOrders()}
              ...
              {section === "ventas" && (
                <section>
                  <div className="mb-6">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Consulta las ventas y genera una boleta interna
                      imprimible.
                    </p>
                  </div>
              ...
              {section === "productos" && (
                <ProductsSection
                  ...
                  onFeatured={(product) => void handleFeatured(product)}
                  disabled={saving}
                />
              )}
                ...
                <ContentSection
                  ...
                  onSave={async () => {
                    ...
                    try {
                      ...
                      notify("Contenido publicado correctamente.");
                    } catch {
              ...
              {section === "usuarios" && (
                <AdminUsersSection
                  ...
                  disabled={saving}
                  onBan={(account) => void handleBan(account)}
      ...
      {selectedOrder && (
        <Modal
          title={`Pedido #${String(selectedOrder.id).padStart(5, "0")}`}
        ...
        >
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            <div>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        {item.selectedSize} · {item.quantity} unidad(es)
                      </p>
                    </div>
              ...
              <div className="mt-5 rounded-2xl bg-gray-50 p-4 text-sm leading-6">
                <strong>Entrega</strong>
                <p>
                  {selectedOrder.department}, {selectedOrder.province},{" "}
                  {selectedOrder.district}
                </p>
                <p>{selectedOrder.deliveryAddress}</p>
                {selectedOrder.addressReference && (
                  <p className="text-gray-500">
                    Ref.: {selectedOrder.addressReference}
                  </p>
                )}
              </div>
            </div>
            <aside className="rounded-3xl border border-violet-100 bg-violet-50 p-5">
              <p className="font-bold">{selectedOrder.customerName}</p>
              <p className="text-sm text-gray-500">
                {selectedOrder.customerEmail}
              </p>
              <div className="my-5 grid gap-2 border-y border-violet-100 py-5 text-sm">
                ...
                <div className="flex justify-between">
                  <span>Envío</span>
                  <span>{money(selectedOrder.shippingCost)}</span>
                </div>
              ...
              {selectedOrder.status === "cancelled" ? (
                <div className="mt-2 rounded-2xl bg-red-100 px-4 py-3 text-sm font-bold text-red-700">
                  Pedido cancelado
                </div>
              ) : (
                <>
              ...
              <Button
                ...
                className="mt-3 w-full cursor-pointer rounded-full bg-zinc-200 hover:bg-zinc-300 text-gray-800 transition-colors"
              >
                <FiFileText /> Generar boleta
              </Button>
              {selectedOrder.status !== "cancelled" && (
                <Button
                  onClick={() => void handleStatus(selectedOrder, "cancelled")}
                  className="mt-3 w-full cursor-pointer rounded-full bg-red-500 text-white hover:bg-red-600 transition-colors"
                >
                  <FiX /> Cancelar pedido
                </Button>
              )}
      ...
      {editingProduct && (
        <ProductEditorWithGallery
          ...
          onSave={async (payload) => {
            ...
            setError(null);
            try {
              if (editingProduct === "new") await createProduct(payload);
              else await updateProduct(editingProduct.id, payload);
              ...
              const loaded = await getAdminProducts();
              setProducts(loaded);
            } catch {
              setError(
                "No se pudo guardar el producto. Revisa los campos y la migración de Neon.",
              );
            } finally {
              setSaving(false);
            }
...
function ProductsSection({
  ...
  products: Product[];
  onEdit: (product: Product | "new") => void;
    ...
    <section>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Crea, edita, retira productos y elige los destacados del mes.
          </p>
        </div>
        <Button
          onClick={() => onEdit("new")}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {visible.map((product) => (
          <article
            key={product.id}
            className={`overflow-hidden rounded-3xl border bg-white ${product.isActive === false ? "opacity-60" : ""}`}
          >
            <img
              src={product.image}
              alt={product.name}
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500">
                    Stock: {product.stock} · {money(product.price)}
                  </p>
                </div>
                <button
                  ...
                  className={`shrink-0 rounded-full px-3 py-1 text-xs font-bold ${product.isFeaturedMonth ? "bg-violet-600 text-white" : "bg-gray-100 text-gray-500"}`}
                >
                  Mes
                </button>
              </div>
              <div className="mt-4 flex gap-2">
                ...
                <Button
                  variant="outline"
                  disabled={disabled}
                  title={product.isActive === false ? "Restaurar" : "Retirar"}
...
function ProductEditorWithGallery({
  product,
  saving,
  onClose,
  onSave,
  ...
  const [form, setForm] = useState<ProductInput>(
    product ? (({ id: _id, ...rest }) => rest)(product) : emptyProduct,
  );
  ...
  const submit = (event: FormEvent) => {
    ...
    void onSave({
      ...form,
      galleryImages: gallery.map((image) => image.trim()).filter(Boolean),
    });
  };
      ...
      <form onSubmit={submit} className="grid gap-5">
        ...
        <div className="rounded-3xl border border-violet-100 bg-violet-50 p-4">
          ...
          <p className="mt-1 text-xs text-violet-700">
            Agrega hasta 4 imagenes adicionales. La imagen principal siempre
            aparece primero.
          </p>
              ...
              >
                {image ? (
                  <img
                    ...
                    className="h-16 w-18 rounded-2xl bg-white object-cover"
                  />
                ) : (
                  <div className="h-16 w-18 rounded-2xl border border-dashed border-violet-200 bg-white" />
                )}
                <Field label={`Imagen extra ${index + 1}`}>
                  <Input
                    value={image}
                    onChange={(event) =>
                      setGalleryImage(index, event.target.value)
                    }
                    placeholder="/imagenes/producto-detalle.webp"
                  />
                </Field>
              </div>
            ))}
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <NumberField
            label="Peso (g)"
            value={form.peso}
            onChange={(value) => set("peso", value)}
            min={0}
          />
          <NumberField
            label="Entrega (dias)"
            value={form.diasEntrega}
            onChange={(value) => set("diasEntrega", value)}
            min={1}
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Instrucciones de cuidado">
          <textarea
            value={form.instruccionesCuidado}
            onChange={(event) =>
              set("instruccionesCuidado", event.target.value)
            }
            rows={2}
            className="rounded-2xl border px-3 py-2 text-sm"
          />
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Toggle
            label="En promocion"
            checked={form.onSale}
            onChange={(value) => set("onSale", value)}
          />
          <Toggle
            label="Nuevo"
            checked={form.isNew}
          ...
          <Toggle
            ...
            checked={form.isFeaturedMonth ?? false}
            onChange={(value) => set("isFeaturedMonth", value)}
          />
        </div>
  ...
  return (
    ...
    >
      <form onSubmit={submit} className="grid gap-5">
        ...
        <Field label="Ruta o URL de imagen">
          <Input
            value={form.image}
            onChange={(event) => set("image", event.target.value)}
            placeholder="/imagenes/producto.webp"
...
function ContentSection({
  ...
  const setArray = (
    key: "homeDesktopImages" | "homeMobileImages",
    index: number,
    value: string,
  ) =>
    setSettings((current) => ({
  ...
  return (
    <section>
      <div className="mb-6">
        <h2 className="text-2xl font-bold">Contenido visual</h2>
        <p className="mt-1 text-sm text-gray-500">
          Actualiza rutas dentro de public o URLs HTTPS. Los cambios se muestran
          en la tienda.
        </p>
      </div>
      <div className="grid gap-6">
        <article className="rounded-3xl border bg-white p-5 sm:p-6">
          <h3 className="font-bold">Carrusel de inicio</h3>
          <p className="mb-5 mt-1 text-xs text-gray-500">
            Tres imágenes de escritorio y sus equivalentes móviles.
          </p>
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="grid gap-3">
              <p className="text-sm font-semibold">Escritorio</p>
              {settings.homeDesktopImages.map((image, index) => (
                <ImageInput
                  ...
                  value={image}
                  onChange={(value) =>
                    setArray("homeDesktopImages", index, value)
                  }
                />
              ))}
            </div>
            <div className="grid gap-3">
              <p className="text-sm font-semibold">Móvil</p>
              {settings.homeMobileImages.map((image, index) => (
                <ImageInput
                  ...
                  value={image}
                  onChange={(value) =>
                    setArray("homeMobileImages", index, value)
                  }
                />
              ))}
            </div>
        ...
        <article className="rounded-3xl border bg-white p-5 sm:p-6">
          <h3 className="font-bold">Portadas y promoción</h3>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            <ImageInput
              label="Portada de Productos"
              value={settings.productsHeaderImage}
              onChange={(value) =>
                setSettings((current) => ({
                  ...current,
                  productsHeaderImage: value,
                }))
              }
            />
            <ImageInput
              label="Promociones (escritorio)"
              value={settings.promotionsHeaderImage}
              onChange={(value) =>
                setSettings((current) => ({
                  ...current,
                  promotionsHeaderImage: value,
                }))
              }
            />
            <ImageInput
              label="Promociones (móvil)"
              value={settings.promotionsMobileImage}
              onChange={(value) =>
                setSettings((current) => ({
                  ...current,
                  promotionsMobileImage: value,
                }))
              }
            />
            <ImageInput
              label="Promoción de inicio"
              value={settings.homePromotionImage}
              onChange={(value) =>
                setSettings((current) => ({
                  ...current,
                  homePromotionImage: value,
                }))
              }
            />
          </div>
          <div className="mt-5 max-w-md">
            <Field label="La promoción termina en">
              <Input
                type="datetime-local"
                value={settings.promotionEndAt.slice(0, 16)}
                onChange={(event) =>
                  setSettings((current) => ({
                    ...current,
                    promotionEndAt: new Date(event.target.value).toISOString(),
                  }))
                }
              />
            </Field>
        ...
        <div className="flex justify-end">
          <Button
            disabled={saving}
            onClick={() => void onSave()}
            className="rounded-full bg-violet-600 px-7 hover:bg-violet-700"
          >
            {saving ? <FiLoader className="animate-spin" /> : <FiCheck />}{" "}
            Publicar cambios
          </Button>
    ...
    <section>
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Consulta clientes, compras y controla el acceso.
          </p>
        </div>
        <Button
      ...
      <div className="overflow-hidden rounded-3xl border bg-white">
        {users.map((account) => (
          <article
            key={account.userId}
            className="grid gap-3 border-b p-5 last:border-0 lg:grid-cols-[1.4fr_1fr_130px_130px] lg:items-center"
          >
            <div className="min-w-0">
            ...
            <span
              className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${account.banned ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}
            >
            ...
            {account.userId !== currentUserId ? (
              <Button
                ...
                className={`rounded-full ${account.banned ? "text-emerald-700" : "text-red-600"}`}
              >
      ...
      {showCreate && (
        ...
        >
          <div className="rounded-2xl border border-violet-100 bg-violet-50 p-5 text-sm leading-6 text-gray-700">
            <p className="font-bold text-violet-800">
              Las contraseñas no deben pasar por el navegador del panel.
            </p>
            <p className="mt-2">
              Crea el usuario en{" "}
              <strong>Neon Console → Auth → Users → Create user</strong>. Luego
              vuelve aquí y pulsa actualizar. Aparecerá automáticamente en esta
              lista.
            </p>
          </div>
...
function CreateUserModal({
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(
        message.includes("already") || message.includes("existe")
          ? "Ese correo ya existe. Usa otro correo o revisa la lista de usuarios."
          : message,
      );
    } finally {
      setCreating(false);
      ...
      <form onSubmit={submit} className="grid gap-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-violet-700">
                El usuario podra iniciar sesion con este correo y contraseña.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col-reverse gap-3 border-t pt-5 sm:flex-row sm:justify-end">
          ...
          <Button
            ...
            className="rounded-full bg-violet-600 hover:bg-violet-700"
          >
            {creating ? <FiLoader className="animate-spin" /> : <FiUserPlus />}{" "}
            Crear usuario
          </Button>
        </div>
      </form>
  ...
  return (
    <div className="grid grid-cols-[64px_1fr] items-center gap-3">
      <img
        src={value}
        alt=""
        className="h-14 w-16 rounded-xl bg-gray-100 object-cover"
      />
      <Field label={label}>
        <Input
          value={value}
